...
# Использование словаря для хранения данных каждого пользователя
user_data = {}
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id not in user_data:
        user_data[user_id] = {"items_to_search": {}, "favorite_items": {}}
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "add":
        ...
        if not user_data[user_id]["items_to_search"]:
            await query.edit_message_text("Список пуст.", reply_markup=main_keyboard())
            return
        keyboard = [
            [InlineKeyboardButton(f"❌ {name}", callback_data=f"delete|{name}") for name in user_data[user_id]["items_to_search"].keys()]
        ]
        keyboard.append([InlineKeyboardButton("🔙 Назад", callback_data="back")])
        await query.edit_message_text("Ваши предметы:", reply_markup=InlineKeyboardMarkup(keyboard))
    elif query.data.startswith("delete|"):
        name = query.data.split("|", 1)[1]
        if name in user_data[user_id]["items_to_search"]:
            del user_data[user_id]["items_to_search"][name]
            await query.edit_message_text(f"Удалено: {name}", reply_markup=main_keyboard())
    elif query.data == "scan":
        await scan(query, context, user_id)
    elif query.data == "favorites":
        if not user_data[user_id]["favorite_items"]:
            await query.edit_message_text("Избранное пусто.", reply_markup=main_keyboard())
            return
        keyboard = [
            [InlineKeyboardButton(f"❌ {name}", callback_data=f"remove_favorite|{name}") for name in user_data[user_id]["favorite_items"].keys()]
        ]
        keyboard.append([InlineKeyboardButton("🔙 Назад", callback_data="back")])
        ...
        name = query.data.split("|", 1)[1]
        if name in user_data[user_id]["favorite_items"]:
            del user_data[user_id]["favorite_items"][name]
...
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if waiting_for_input.get(user_id) == "add":
        ...
        user_data[user_id]["items_to_search"][item_name] = {"min": min_price, "max": max_price}
        ...
        user_data[user_id]["favorite_items"][item_name] = {"min": min_price, "max": max_price}
        await update.message.reply_text(
...
async def scan(update_or_query, context: ContextTypes.DEFAULT_TYPE, user_id):
    found = []
    url = API_URL
    ...
    try:
        ...
        for entry in data:
            ...
            for item_name, price_range in user_data[user_id]["items_to_search"].items():
            ...
            for user_favorites in user_data.values():
                for item_name, price_range in user_favorites["favorite_items"].items():
                    item_set = normalize(item_name)
                    if item_set.issubset(name_set) and min_price:
                        min_price_f = float(min_price)
                        if price_range["min"] <= min_price_f <= price_range["max"]:
